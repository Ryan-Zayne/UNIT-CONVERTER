...
function conversion() {
  ...
  if (inputEl.value > 0) {
    lengthCard.textContent = `${inputEl.value} meters = ${meterConv.toFixed(
      3
    )} feet | ${inputEl.value} feet = ${feetConv.toFixed(3)} meters`;

    volumeCard.textContent = `${inputEl.value} liters = ${literConv.toFixed(
      3
    )} gallons | ${inputEl.value} gallons = ${gallonConv.toFixed(3)} liters`;

    massCard.textContent = `${inputEl.value} kilograms = ${kilogramConv.toFixed(
      3
    )} pounds | ${inputEl.value} pounds = ${poundsConv.toFixed(3)} kilograms`;
  } else {
